...
install_system_deps(){
  if ! have apt-get; then log "apt-get not found; skipping system dependency installation"; return 0; fi
  local SUDO=""
  if [ "$(id -u)" -ne 0 ]; then
    if have sudo; then SUDO=sudo; else log "sudo unavailable; skipping apt dependencies"; return 0; fi
  fi
  $SUDO apt-get update
  $SUDO apt-get install -y --no-install-recommends ca-certificates git curl unzip make cmake ninja-build build-essential pkg-config python3 python3-pip clang llvm lldb binutils-arm-none-eabi openjdk-21-jdk libssl-dev libcurl4-openssl-dev zlib1g-dev
}
...
download_verify(){ local url="$1" dst="$2" sha="$3"; if [ ! -f "$dst" ]; then curl -fL --retry 3 --retry-delay 2 "$url" -o "$dst"; fi; printf '%s  %s\n' "$sha" "$dst" | sha256sum -c -; }
...
write_env(){ cat > "$TOOLS/env.sh" <<'ENV'
#!/usr/bin/env bash
...
install_ctr; install_3dstool; install_ghidra; install_azahar; write_env
printf 'platform=nintendo-3ds\nghidra=12.1.3\nazahar=2126.1.1\n' > "$TOOLS/versions.txt"
